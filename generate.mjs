...
<dialog id="confirmDialog" class="add-dialog">
  <div class="add-form">
    <h2>¿Eliminar este botón?</h2>
    <p class="duration-hint">Esta acción no se puede deshacer.</p>
    <div class="dialog-actions">
      <button type="button" id="confirmCancelBtn" class="ghost-btn">Cancelar</button>
      <button type="button" id="confirmOkBtn" class="primary-btn">Eliminar</button>
    </div>
  </div>
</dialog>
  ...
  (function () {
    ...
    padEl.addEventListener('click', function (evt) {
      ...
      if (deleteTarget) {
        ...
        var wrapForDelete = deleteTarget.closest('.btn-wrap');
        askConfirm().then(function (ok) {
          if (!ok) return;
          if (key.indexOf('custom-') === 0) {
            removeCustomButton(key, wrapForDelete, audioEls[key]);
          } else {
            deleteBuiltin(key);
          }
        });
